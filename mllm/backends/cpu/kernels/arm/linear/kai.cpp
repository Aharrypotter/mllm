...
bool KaiLinear_f32_qai8dxp_qsi4c32p_mxk_nxk::matmul_shared_input(const float* __restrict__ lhs_fp32,
                                                                 const SharedInputProjection* projections,
                                                                 size_t projection_count, void* workspace, int M, int K,
                                                                 KaiLinear_f32_qai8dxp_qsi4c32p_mxk_nxk::Tiles tile_cfg,
                                                                 int thread_count) {
  if (lhs_fp32 == nullptr || projections == nullptr || projection_count < 2 || workspace == nullptr || M <= 0 || K <= 0
      || thread_count <= 0) {
    return false;
  }
  ...
  kai_run_lhs_quant_pack_qai8dxp_f32(M, K, ukernel.get_mr(), ukernel.get_kr(), ukernel.get_sr(), 0, lhs_fp32, K * sizeof(float),
                                     workspace);

  const size_t m_step = static_cast<size_t>(ukernel.get_m_step());
  const size_t n_step = static_cast<size_t>(ukernel.get_n_step());
  const size_t m_tiles = (static_cast<size_t>(M) + m_step - 1) / m_step;
  size_t total_tiles = 0;
  for (size_t projection_index = 0; projection_index < projection_count; ++projection_index) {
    const size_t n_tiles = (static_cast<size_t>(projections[projection_index].n) + n_step - 1) / n_step;
    if (n_tiles > std::numeric_limits<size_t>::max() / m_tiles) { return false; }
    const size_t projection_tiles = m_tiles * n_tiles;
    if (projection_tiles > std::numeric_limits<size_t>::max() - total_tiles) { return false; }
    total_tiles += projection_tiles;
  }
  ...
  MLLM_CONDITIONAL_PARALLEL_FOR(thread_count > 1, thread_count, global_tile, 0, total_tiles, 1, {
    ...
    for (; projection_index < projection_count; ++projection_index) {
      const size_t projection_n_tiles = (static_cast<size_t>(projections[projection_index].n) + n_step - 1) / n_step;
      const size_t projection_tiles = m_tiles * projection_n_tiles;
    ...
    if (projection_index < projection_count) {
      ...
      const size_t projection_n_tiles = (static_cast<size_t>(projection.n) + n_step - 1) / n_step;
      const int m_index = static_cast<int>((local_tile / projection_n_tiles) * m_step);
      const int n_index = static_cast<int>((local_tile % projection_n_tiles) * n_step);
      const int actual_m = std::min(M - m_index, static_cast<int>(m_step));
      ...
      const void* lhs_ptr =
          static_cast<const void*>(static_cast<const char*>(workspace) + ukernel.get_lhs_packed_offset(m_index, K));
      const void* rhs_ptr = static_cast<const void*>(reinterpret_cast<const char*>(projection.packed_weight_bias)
                                                     + ukernel.get_rhs_packed_offset(n_index, K, 32));
      float* dst_ptr = reinterpret_cast<float*>(reinterpret_cast<uint8_t*>(projection.dst)
                                                + ukernel.get_dst_offset(m_index, n_index, dst_stride));

      ukernel.run_matmul(actual_m, actual_n, K, 32, lhs_ptr, rhs_ptr, dst_ptr, dst_stride, sizeof(float),
                         -std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
    }
  });
  return true;
...
bool KaiLinear_f32_qai8dxp_qsi4c32p_mxk_nxk::matmul_shared_input_m1(const float* __restrict__ lhs_fp32,
                                                                    const SharedInputProjection* projections,
                                                                    size_t projection_count, void* workspace, int K,
                                                                    KaiLinear_f32_qai8dxp_qsi4c32p_mxk_nxk::Tiles tile_cfg,
                                                                    int thread_count) {
  return matmul_shared_input(lhs_fp32, projections, projection_count, workspace, 1, K, tile_cfg, thread_count);
}
