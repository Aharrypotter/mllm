...
float causalConvPatternValue(std::size_t index, int salt) {
  const auto scaled = static_cast<float>((index * 37U + static_cast<unsigned>(salt) * 11U) % 251U);
  return (scaled - 125.0F) / 64.0F;
}

std::vector<float> makeCausalConvBuffer(std::size_t count, int salt) {
  std::vector<float> buffer(count);
  for (std::size_t index = 0; index < count; ++index) { buffer[index] = causalConvPatternValue(index, salt); }
  return buffer;
}
...
//===----------------------------------------------------------------------===//
TEST_F(KernelTest, CausalDepthwiseConvHistoryFirstK3MatchesScalarReferenceBitwise) {
    ...
    for (int sequence : {1, 2, 28, 225}) {
      for (int channels : {1, 3, 4, 5, 2045, 2048}) {
        for (bool non_zero_history : {false, true}) {
          ...
          const std::vector<float> input = makeCausalConvBuffer(element_count, channels + sequence);
          const std::vector<float> weight = makeCausalConvBuffer(static_cast<std::size_t>(channels) * kKernel, kKernel);
          const std::vector<float> initial_state =
              non_zero_history ? makeCausalConvBuffer(state_count, 19) : std::vector<float>(state_count, 0.0F);
